...
export class InvestmentPortfolioAssetFormComponent implements OnInit {
  ...
  submit(event: Event): void {
    ...
    const fixedIncomeNetAmount = Number(raw.fixedIncomeNetAmount) || 0;
    const fixedIncomeCurrentAmount = fixedIncomeNetAmount || fixedIncomeGrossAmount || fixedIncomeInvestedAmount;
    const investedAmount = isFixedIncome ? fixedIncomeInvestedAmount : Number(raw.quantity) * Number(raw.averagePrice);
    const currentAmount = isFixedIncome ? fixedIncomeCurrentAmount : Number(raw.quantity) * Number(raw.currentPrice);
    ...
    const payload: NewInvestmentPortfolioAsset = {
      ...
      currentPrice: isFixedIncome ? fixedIncomeCurrentAmount : Number(raw.currentPrice),
      purchaseDate: raw.purchaseDate || null,
      targetAllocation: Number(raw.targetAllocation) || 0,
      dividendYield: Number(raw.dividendYield) || 0,
      annualIncome: isFixedIncome ? Math.max(0, currentAmount - investedAmount) : Number(raw.annualIncome) || 0,
      ...
      fixedIncomeGrossAmount: isFixedIncome ? fixedIncomeGrossAmount || fixedIncomeCurrentAmount : null,
      fixedIncomeNetAmount: isFixedIncome ? fixedIncomeNetAmount || null : null,
